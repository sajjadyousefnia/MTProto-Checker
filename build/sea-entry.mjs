...
// build asset map: path → { data, mime }
const assetMap = new Map();
for (const a of assets) {
    assetMap.set(a.path, a);
}
// map / → /index.html
const indexHtml = assetMap.get('/index.html');
if (indexHtml) assetMap.set('/', indexHtml);

// serve embedded assets with disk fallback
app.use((req, res, next) => {
    if (req.method !== 'GET' && req.method !== 'HEAD') return next();
    const a = assetMap.get(req.path);
    if (!a) return next();
